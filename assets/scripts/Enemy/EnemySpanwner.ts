...
export class EnemySpanwner extends Component {
    ...
    private SpanwnerDelay:number=5;//怪物生成延迟

    //状态机AI相关参数
    private interval:number=1;//状态机AI思考间隔
    ...
    update(deltaTime: number) {
        ...
        if(this.curDelay>this.SpanwnerDelay){
            ...
            if(this.enemyPool.size()>0){
                ...
                // console.log("333",enemynode.getComponent("Enemy").Enemyname);
                if(enemynode.getComponent("Enemy").Enemyname!=null){
                    enemynode.getComponent("Enemy").reset();
                    // console.log("444",enemynode.getComponent("Enemy").getblood());
                }
            }
            else{
